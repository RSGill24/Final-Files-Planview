...
=========================================================================
    ...
    -- For Initiatives: use [Strategy Type] as source for Execution Type - Legacy
    -- Strategy Type confirmed in Initiatives view by Bert
    -- Bert confirmed Strategy Type column is now in the Initiatives view
    SET @sql = N'
    UPDATE ' + @i + N' SET
        [Demand Type - Legacy]    = COALESCE([Demand Type - Legacy],    [Demand Type]),
        [Execution Type - Legacy] = COALESCE([Execution Type - Legacy], [Strategy Type])
    ...
    -- Derives new target Demand Type from Flow Type (populated by SP2).
    -- Demand Type - Legacy is already set above in Section 5.
    ...
    SET @sql = N'
    IF NOT EXISTS (SELECT 1 FROM sys.columns
                   WHERE object_id = OBJECT_ID(''' + @i + N''')
                   AND name = N''Demand Type'')
        ALTER TABLE ' + @i + N' ADD [Demand Type] NVARCHAR(255)';
    EXEC sp_executesql @sql;
